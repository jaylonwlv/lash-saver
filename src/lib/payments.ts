...
export function paymentAppUrl(
  ...
  // Cash App ignores an amount in the link (it opens the app's home screen), so
  // open the pro's profile and the pay page tells the client what to send.
  if (handle.app === "cashapp") return `https://cash.app/${handle.handle}`;
  if (handle.app === "venmo") {
